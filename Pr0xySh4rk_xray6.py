...
# Gather the best N working configs for each group (per source now)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=thread_pool_size) as executor:
        ...
        if test_type == "tcp+http":
            tcp_futures = [futures[0] for idx, futures in futures_map.items()
                           if futures and outbounds[idx].get("original_config", "").split("://")[0].lower() not in ("warp", "wireguard")]
...
# ---------------------------
# Round-robin diversifying by source
# ---------------------------
def diversify_configs_round_robin(configs: List[Dict[str, Any]]) -> List[str]:
    # Group configs by subscription source
    groups: Dict[str, List[Dict[str, Any]]] = {}
    for conf in configs:
        source = conf.get("source", "unknown")
        groups.setdefault(source, []).append(conf)
    
    # Sort each group by combined_delay and limit the number per source
    for src, conf_list in groups.items():
        conf_list.sort(key=lambda x: x.get("combined_delay", float('inf')))
        groups[src] = conf_list[:BEST_CONFIGS_LIMIT]
    
    # Round-robin interleaving of configs from each source
    round_robin_list: List[Dict[str, Any]] = []
    group_keys = list(groups.keys())
    while any(groups[src] for src in group_keys):
        for key in group_keys:
            if groups[key]:
                round_robin_list.append(groups[key].pop(0))
    
    # Rename the configs with a new tag that includes protocol abbreviation and sequence number
    renamed_configs = []
    protocol_map = {
        "ss": "SS",
        "vless": "VL",
    ...
    for idx, config_dict in enumerate(round_robin_list, start=1):
        ...
        try:
            ...
            abbr = "CFG"
        new_tag = f"🔒Pr0xySh4rk🦈{abbr}{idx:02d}"
        if "#" in config:
...
# ---------------------------
# Rename and limit configs by protocol (old method, now replaced)
# ---------------------------
def rename_configs_by_protocol(configs: List[Dict[str, Any]]) -> List[str]:
    # This function is retained for reference but is not used in the new round-robin diversification.
    protocol_map = {
        "ss": "SS",
        "vless": "VL",
        "vmess": "VM",
...
def main():
    ...
    # Diversify and interleave configs from different sources using round-robin
    renamed_final_outbounds = diversify_configs_round_robin(tested_outbounds)
    print(f"Renamed and diversified (round-robin) configs: {len(renamed_final_outbounds)}")
