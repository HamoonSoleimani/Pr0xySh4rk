...
class XrayCore:
    ...
    def startFromJSON(self, json_config_string: str):
        ...
        try:
            ...
            # Wait a bit longer for xray-core to start
            time.sleep(2)
            if self.process.poll() is not None:
                stdout, stderr = self.process.communicate()
                print("xray-core stdout:\n", stdout.decode())
                print("xray-core stderr:\n", stderr.decode())
                raise subprocess.SubprocessError(
                    f"xray-core exited immediately with code: {self.process.returncode}"
                )
...
def measure_latency_icmp_proxychains(target_host: str = "www.google.com",
    ...
    try:
        ...
        if match:
            ...
            print("Failed to parse fping output.")
            return float('inf')
...
def measure_xray_latency_http(proxy: str, timeout: int = 5) -> float:
    """Measures HTTP latency using a diverse set of URLs with retries."""
    test_urls = [
    ...
    for url in test_urls:
        for attempt in range(3):  # Retry each URL
            try:
    ...
    if successful_requests > 0:
        print(f"Best HTTP latency: {min_latency:.2f} ms (from {successful_requests} successful requests)")
        return min_latency
    else:
...
def real_delay_test_outbound(outbound_config: dict) -> float:
    ...
    try:
        ...
        time.sleep(2)  # Allow xray-core to start
...
def merge_configs(base_config: Dict[str, Any], outbound_lists: List[List[Dict[str, Any]]], all_tags: set) -> Dict[str, Any]:
    ...
    for ob in merged_outbounds:
        ...
        if ob.get("protocol") == "selector":
            ...
            all_tags.update(urltest_outbounds)
    ...
    if any("detour" in ob and ob["detour"] == "Hiddify Warp" for ob_list in outbound_lists for ob in ob_list):
        if not any(ob.get("tag") == "Hiddify Warp" for ob in base_config["outbounds"]):
...
def save_config(outbounds: List[Dict[str, Any]], filepath: str = "merged_config.txt"):
    try:
        output_lines = []
        for ob in outbounds:
        ...
        with open(filepath, "w") as outfile:
            for line in output_lines:
                outfile.write(line + "\n")
        print(f"Merged configs saved to {filepath} in single-line format.")
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--no-base64", action="store_true", dest="no_base64_output",
                        help="Not applicable to single-line output")
    ...
    if not subscription_urls:
        ...
        save_config([], filepath=args.output)
        return
    ...
    if test_type == "tcp+http":
        ...
        temp_tested_outbounds = all_parsed_outbounds[:]

        for test in tests_to_run:
            print(f"\n=== Starting {test.upper()} test pass ===")
            single_test_pass(temp_tested_outbounds, test, args.threads, args.test_proxy, args.repetitions)
            if test == "tcp":
                temp_tested_outbounds = [ob for ob in temp_tested_outbounds if ob.get("tcp_delay", float('inf')) != float('inf')]
            elif test == "http":
                temp_tested_outbounds = [ob for ob in temp_tested_outbounds if ob.get("http_delay", float('inf')) != float('inf')]
            elif test == "real":
                temp_tested_outbounds = [ob for ob in temp_tested_outbounds if ob.get("xray_delay", float('inf')) != float('inf')]
        tested_outbounds = temp_tested_outbounds
    ...
    save_config(renamed_outbounds, filepath=args.output)
