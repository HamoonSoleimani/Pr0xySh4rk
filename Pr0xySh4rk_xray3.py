    ...
    for test_url in TEST_URLS:
        ...
        for i in range(repetitions):
            ...
            try:
                ...
                # Check if response has content and is successful
                if response.status_code == 200 and response.content:
                    times.append(elapsed)
                    successes += 1
                    print(f"    [{server}:{port}] {test_url} Rep {i+1}: {elapsed:.2f} ms")
                else:
                    print(f"    [{server}:{port}] {test_url} Rep {i+1} invalid response: {response.status_code}")
            except requests.exceptions.RequestException as e:
                print(f"    [{server}:{port}] {test_url} Rep {i+1} failed: {type(e).__name__}")

        # Store results for this test URL
        if times:
            avg_time = sum(times) / len(times)
            success_rate = successes / repetitions
            website_results.append((avg_time, success_rate))
            print(f"  Average delay for {test_url}: {avg_time:.2f} ms, Success rate: {success_rate:.2f}")
        else:
            print(f"  All trials failed for {test_url}")

    # Calculate overall results
    if website_results:
        # Only consider a config valid if at least one test URL had successful requests
        valid_times = [t for t, r in website_results if r > 0]
        if valid_times:
            # Calculate weighted average based on success rates
            weighted_sum = sum(t * r for t, r in website_results)
            total_weight = sum(r for _, r in website_results)
            if total_weight > 0:
                ob["http_delay"] = weighted_sum / total_weight
                print(f"HTTP Test for {server}:{port} completed. Weighted Average: {ob['http_delay']:.2f} ms")
            else:
                ob["http_delay"] = float('inf')
                print(f"HTTP Test for {server}:{port} failed due to zero success rate.")
        else:
            ob["http_delay"] = float('inf')
            print(f"HTTP Test for {server}:{port} failed with no valid times.")
    else:
        ob["http_delay"] = float('inf')
        print(f"HTTP Test for {server}:{port} failed with no website results.")
...
async def udp_test_outbound(ob: Dict[str, Any]) -> None:
    ...
    if not server or not port:
        if is_wireguard:
            # For WireGuard/WARP, some configs might not have explicit server:port
            print(f"UDP Test: No server/port for WG/WARP, checking if config is valid: {config_line}")
            try:
                # Parse config to see if it has endpoints or other required fields
                # This is a simplified check, real implementation would parse the config properly
                if "#" in config_line:
                    # Some basic validation that this is likely a valid config
    ...
    try:
        ...
        # Send a test packet (we don't expect a response, just testing packet send)
        sock = transport.get_extra_info('socket')
        sock.sendto(b'\x00\x00\x00\x00', (resolved_ip, port))
...
def single_test_pass(outbounds: List[Dict[str, Any]],
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=thread_pool_size) as executor:
        ...
        for index, ob in enumerate(outbounds):
            ...
            if test_type == "tcp+http":
                if protocol in ("warp", "wireguard"):
                    ...
                    # We'll add HTTP test after TCP completes
            elif test_type == "tcp":
        ...
        # First complete all TCP tests before starting HTTP tests
        if test_type == "tcp+http":
            tcp_futures = [futures[0] for index, futures in futures_map.items()
                          if futures and outbounds[index].get("original_config", "").split("://")[0].lower() not in ("warp", "wireguard")]
            ...
            for future in concurrent.futures.as_completed(tcp_futures):
                ...
                try:
                    ...
                    print(f"Exception during TCP test: {e}")
            ...
            for index, futures_list in list(futures_map.items()):
                ...
                if protocol not in ("warp", "wireguard") and outbounds[index].get("tcp_delay", float('inf')) != float('inf'):
                    # If TCP test passed, add HTTP test
...
def main():
    ...
    if args.test == "tcp+http":
        # Separate WireGuard/WARP configs
