...
class XrayCore:
    ...
    def startFromJSON(self, json_config_string: str):
        ...
        try:
            ...
            print("Failed to start xray-core: 'xray' executable not found. Ensure it's in your PATH or in the 'xray' subdirectory.")
            self.process = None
...
def measure_latency_icmp_proxychains(target_host: str = "www.google.com",
    ...
    command = ["proxychains4", "-q", "-f", config_path, "fping", "-c", str(count), target_host] # added -q for quiet
    try:
        ...
        if isinstance(e, subprocess.CalledProcessError):
           print(f"fping output: {e.output}") # Print fping output for debuging
        return float('inf')
    finally:
        try:
...
def real_delay_test_outbound(outbound_config: dict) -> float:
    ...
    try:
        ...
        time.sleep(3)  # Increased sleep time for Xray stability
